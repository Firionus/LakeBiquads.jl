...
function compareReference(refName, testFilter; fStart = nothing, fStop=22e3, plot=false)
    ...
    for i in eachindex(fIn)
        ...
        elseif fIn[i] >= fStart
            ...
            if isapprox(phaseIn[i], angle(freqz(testFilter, fIn[i], fs))/pi*180, atol=1) != true
...
@testset "all Tests" begin
    @testset "Reference comparisons" begin
        ...
        #now the LowHighPass tests, take care to adjust fStart/fStop individually to surpress Noise
        @test_broken compareReference("HPBS48 1k.csv", HighPass(:BS, 1e3, 48), fStart=500, plot=false) #fails hard
    ...
    @testset "Batch conversion" begin
        ...
        #TODO
        #read resulting file and compare to single calculated filters
    end
end
